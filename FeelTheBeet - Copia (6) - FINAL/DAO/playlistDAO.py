...
class PlaylistDAO:
    #consulta pelo id
    @staticmethod
    def get_id_dao(id): # função que busca no db o id da playlist
        return Playlist.query.get(id)
    #create
    @staticmethod
    def addPlaylist_dao(nome): #função q adiciona uma Playlist no db
    ...
    #read
    @staticmethod
    def getPlaylist_dao(id): #pega o id da playlist
        if id:
            return Playlist.query.filter_by(id = id).first() #consulta no db, filtrando pelo id
        return id

    #update
    @staticmethod #função estatica, ja q esta se relacionando com o db
    def atualizarPlaylist_dao(id,nome): #recebe como parametro o id e nome 
